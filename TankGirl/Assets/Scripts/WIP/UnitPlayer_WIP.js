  ...
  function Update(){
    ...
    if(Input.GetButton("Jump") && control.isGrounded){
      ...
      if(Physics.Raycast(xform.position, -Vector3.up, hit)){
        ...
        var floorContactOrient    = Quaternion.FromToRotation(Vector3.up, curNormal)      ;
        
        arrow.rotation   = floorContactOrient                                         ;
        floorRotation   = Quaternion.Lerp (xform.rotation, floorContactOrient, Time.time * tankOrientSpeed);
